...
bool Cache::access(unsigned int address) {
  unsigned int byteSelectBits = log2(blocksize);
  unsigned int addressTag = address >> (byteSelectBits);
  for (unsigned int i = 0; i < (size/blocksize); i++) {
    if (entries[i] == addressTag) {
      hits++;
      return true;
    }
  }

  for (unsigned int i = (size/blocksize); i > 0; i--) {
    entries[i] = entries[i - 1];
  }
  entries[0] = address >> (byteSelectBits);
  misses++;
  return false;
}
