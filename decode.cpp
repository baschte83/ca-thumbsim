...
LD_ST_Ops decode (const LD_ST_Type data) {
  if (data.instr.class_type.opA == LD_ST_REG_OPA) {
    if (data.instr.class_type.opB == LD_ST_OPB_LDRB) {
      // 315: write code to print ldrb -> A7.7.47
        if (opts.instrs) {
      ...
      // 315: write code to print strb -> A7.7.161
        if (opts.instrs) {
    ...
    if (data.instr.ld_st_imm.op == LD_ST_LDB) {
      // 315: write code to print ldrb -> LDRB (immediate) T1
        if (opts.instrs) {
      ...
      // 315: write code to print strb -> STRB (immediate) T1
        if (opts.instrs) {
...
int decode (const LDRL_Type data) {
  // 315: add code to print ldr -> setbase 10 ???
    if (opts.instrs) {
        cout << "ldrl r" << data.instr.ldrl.rt << ", #" << setbase(10) << data.instr.ldrl.imm << endl;
    }
  return LDRL;
}
